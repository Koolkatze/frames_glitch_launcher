...
class TxSprite extends TxMsg {
  ...
  TxSprite.fromImageBytes(
      {required super.msgCode, required Uint8List imageBytes}) {
    ...
    if ((image.hasPalette && image.palette!.numColors > 16) ||
        !image.hasPalette) {
      ...
      _log.fine(
          '16 or fewer colors in an indexed image, no quantizing required');
    ...
    if (image.width > 640 || image.height > 400) {
      ...
      if (image.palette!.numColors <= 4) {
        ...
        if (numSrcPixels > 128000) {
          ...
          _log.fine(
              'small 4-bit image, no need to scale down pixels, just max extent in height or width');
    ...
    if (image.palette!.numChannels == 3) {
      ...
      // but packaged in a length 256 palette (a bug)
      initialPalette =
          _extractRGB(image.palette!.toUint8List().sublist(0, 4 * numColors));
    } else {
      throw Exception(
          'Image colors must have 3 or 4 channels to be converted to a sprite');
    }
    ...
    if (image.palette!.numColors < 16) {
      ...
      _log.fine(() =>
          'Sprite: $_width x $_height, $_numColors cols, ${pack().length} bytes');
    }
  }
